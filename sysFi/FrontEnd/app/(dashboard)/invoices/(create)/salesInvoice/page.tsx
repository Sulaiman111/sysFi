...
import { paymentService } from "@/lib/api/payment-service" // Add this import
...
const formSchema = z.object({
  ...
  paymentMethod: z.enum(["paid", "unpaid"]), // Keep these values consistent
})
...
export default function CreateInvoicePage() {
  ...
  // Importa el tipo Customer
  const [isLoading, setIsLoading] = useState(false)
  const [customers, setCustomers] = useState<Customer[]>([]) // Especifica el tipo Customer[]
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      // Find the selected customer to get their details
      ...
      if (selectedCustomer && createdInvoice._id) {
        await customerService.addInvoiceToCustomer(selectedCustomer._id, createdInvoice._id);
      }
      
      // If payment is made, handle payment
      if (values.paymentMethod === "paid" && createdInvoice._id) {
        try {
          // Create payment data
          const paymentData = {
            customerId: values.customer,
          ...
          // Add payment to customer's payment list
          await customerService.addPaymentToCustomer(values.customer, payment._id);
      
          // Add payment to invoice
          await invoiceService.addPaymentToInvoice(createdInvoice._id, {
          ...
          console.log('Payment added to customer payment list:', payment._id);
        } catch (error) {
      ...
      if (selectedCustomer && values.paymentMethod !== "paid") {
        const updatedBalance = (selectedCustomer.balanceDue || 0) + total;
        await customerService.updateCustomer(selectedCustomer._id, {
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="Additional notes for the customer" className="resize-none" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
