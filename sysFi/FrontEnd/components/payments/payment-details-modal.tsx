...
export function PaymentDetailsModal({ paymentId, isOpen, onClose }: PaymentDetailsModalProps) {
  ...
  useEffect(() => {
    const fetchPaymentDetails = async () => {
      ...
      try {
        setIsLoading(true);
        //@ts-ignore
        const paymentData: Payment = await paymentService.getPaymentById(paymentId);
        setPayment(paymentData);

        if (paymentData?.customerId) {
          try {
            const customerId = typeof paymentData.customerId === 'object' 
              ? paymentData.customerId._id 
  ...
  const handlePrint = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Payment Details - ${payment?.paymentId}</title>
          <style>
        ...
        <body>
          ...
          <div class="grid">
            <div class="section">
              ...
              <p>Method: ${payment?.method === 'cash' ? 'Cash' : 'Cheque'}</p>
          ...
          ${payment?.method === 'check' && payment.cheques.length > 0 ? `
            <div class="section">
          ...
          ${payment?.invoiceId ? `
            <div class="section">
              ...
              ${payment.invoice ? `
                <p>Invoice Amount: ${formatCurrency(payment.invoice.amount)}</p>
                <p>Invoice Status: ${payment.invoice.status}</p>
              ` : ''}
            </div>
          ` : ''}
          ${payment?.notes ? `
      ...
      <DialogContent className="max-w-3xl max-h-[90%] overflow-auto">
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            {payment.method === 'check' && payment.cheques && payment.cheques.length > 0 && (
              <div>
            ...
            {payment.invoiceId && (
              <div>
                ...
                {payment.invoice && (
                  <>
                    <p>Invoice Amount: {formatCurrency(payment.invoice.amount)}</p>
                    <p>Invoice Status: <Badge>{payment.invoice.status}</Badge></p>
                  </>
                )}
             ...
             {payment.notes  !== '' && (
                  <div className="mt-6 border-t pt-4">
                    ...
                    <div className=" p-4 rounded-lg border">
                      <p dir="rtl" className="whitespace-pre-wrap text-gray-300">{payment.notes}</p>
                    </div>
                  </div>
                )}
          ...
          <div className="text-center py-4">No payment details found</div>
        )}
      </DialogContent>
    </Dialog>
  );
